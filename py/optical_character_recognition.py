...
class ComputerVision():
    ...
    def __init__(self, **kwargs):
        ...
        self.tesseract_tessdata_dir = ""   # "/usr/share/tessdata" # File PATH：用戶自定義指定本地訓練完畢的數據集文檔（tessdata）的儲存路徑位置，使用 Tesseract 識別，需要使用訓練好的數據集，如果不指定該參數，則預設使用系統預設的數據集文檔目錄 Specify the location of tessdata path.
        self.tesseract_user_words = ""  # File PATH：用戶自定義指定的詞庫（字典）文檔的存儲位置，用戶字典可以包含一些特定的詞匯，以提高對特定詞匯的識別準確性，用戶字典文件應爲純文本文檔，每行一個詞匯 Specify the location of user words file.
